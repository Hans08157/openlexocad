...
class Seitenschutz(lx.Wall):
    _classID = Base.GlobalId("{F24193CE-6D4E-4DD7-B5E8-43DDCF94D2D1}")

    _propHeader = "_04SeitenschutzFürWandöffnungen"
    _propGroupName = "_04SeitenschutzFürWandöffnungenParameter"
    ...
    def getGlobalClassId(self):
        return Seitenschutz._classID

    def __init__(self, aArg):
        lx.Wall.__init__(self, aArg)
        self.registerPythonClass("Seitenschutz", "OpenLxApp.Wall")

        self._insidePropUpdate = False

        self.setPropertyHeader(lxstr(Seitenschutz._propHeader), 524)
        self.setPropertyGroupName(lxstr(Seitenschutz._propGroupName), 525)

        self._length = self.registerPropertyDouble(Seitenschutz._lengthPropName,
                                                   1.0,
                                                   lx.Property.VISIBLE,
                                                   lx.Property.EDITABLE,
                                                   505)

        self._width = self.registerPropertyDouble(Seitenschutz._widthPropName,
                                                  0.2,
                                                  lx.Property.VISIBLE,
                                                  lx.Property.EDITABLE,
                                                  502)

        self._height = self.registerPropertyDouble(Seitenschutz._heightPropName,
                                                   2.0,
                                                   lx.Property.VISIBLE,
                                                   lx.Property.EDITABLE,
                                                   503)
    ...
    def onPropertyChanged(self, aPropertyName):
        if not self._insidePropUpdate:
            ...
            if aPropertyName == Seitenschutz._lengthPropName:
                self.setLength(self._length.getValue())
            elif aPropertyName == Seitenschutz._widthPropName:
                self.setWidth(self._width.getValue())
            elif aPropertyName == Seitenschutz._heightPropName:
...
if __name__ == "__main__":
    ...
    doc.registerPythonScript(Base.GlobalId("{DD458BDC-391B-435F-BECB-0CB4782283C9}"))

    try:
        cwColor = 3106
        element = Seitenschutz(doc)
        lx.setNewComponentByColorAndName(cwColor, Base.StringTool.toString("Seitenschutz Wandoffnung"), element)
        ...
        prop1_value = Base.StringTool.toStlString(Base.PTranslator.get(524))  # "04_Seitenschutz_Wandöffnung"
        ...
        cmd.CmdSetPropertySetDefinition("Pset_WallCommon", "reference", "04_Seitenschutz_Wandöffnung", element).redo()
