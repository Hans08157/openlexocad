...
class Innenkonsole(lx.Wall):
    _classID = Base.GlobalId("{0BD02F0A-16C4-4A29-A9BB-F2C763ED47DF}")

    _propHeader = "_09InnenkonsoleFürFassadengerüst"
    _propGroupName = "_09InnenkonsoleFürFassadengerüstParameter"
    ...
    def getGlobalClassId(self):
        return Innenkonsole._classID

    def __init__(self, aArg):
        lx.Wall.__init__(self, aArg)
        self.registerPythonClass("Innenkonsole", "OpenLxApp.Wall")

        self._insidePropUpdate = False

        self.setPropertyHeader(lxstr(Innenkonsole._propHeader), 536)
        self.setPropertyGroupName(lxstr(Innenkonsole._propGroupName), 537)

        self._length = self.registerPropertyDouble(Innenkonsole._lengthPropName,
                                                   3.0,
                                                   lx.Property.VISIBLE,
                                                   lx.Property.EDITABLE,
                                                   505)

        self._width = self.registerPropertyDouble(Innenkonsole._widthPropName,
                                                  0.3,
                                                  lx.Property.VISIBLE,
                                                  lx.Property.EDITABLE,
                                                  502)

        self._height = self.registerPropertyDouble(Innenkonsole._heightPropName,
                                                   0.3,
                                                   lx.Property.VISIBLE,
                                                   lx.Property.EDITABLE,
                                                   503)
    ...
    def onPropertyChanged(self, aPropertyName):
        if not self._insidePropUpdate:
            ...
            if aPropertyName == Innenkonsole._lengthPropName:
                self.setLength(self._length.getValue())
            elif aPropertyName == Innenkonsole._widthPropName:
                self.setWidth(self._width.getValue())
            elif aPropertyName == Innenkonsole._heightPropName:
...
if __name__ == "__main__":
    ...
    doc.registerPythonScript(Base.GlobalId("{8E6D0626-656A-410C-A6DE-A29614008C1B}"))

    try:
        cwColor = 3113
        element = Innenkonsole(doc)
        lx.setNewComponentByColorAndName(cwColor, Base.StringTool.toString("Innenkonsole Fassadengerüst"), element)
        ...
        prop1_value = Base.StringTool.toStlString(Base.PTranslator.get(536))  # "09_Innenkonsole_Fassadengerüst"
        ...
        cmd.CmdSetPropertySetDefinition("Pset_WallCommon", "reference", "09_Innenkonsole_Fassadengerüst", element).redo()
