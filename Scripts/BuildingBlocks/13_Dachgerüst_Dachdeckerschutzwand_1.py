...
class Gelanderholm(lx.Wall):
    _classID = Base.GlobalId("{7C3E8F14-493F-44D2-A84A-22BD1021A0F3}")

    _propHeader = "_13DachgerüstDachdeckerschutzwand"
    _propGroupName = "_13DachgerüstDachdeckerschutzwandParameter"
    ...
    def getGlobalClassId(self):
        return Gelanderholm._classID

    def __init__(self, aArg):
        lx.Wall.__init__(self, aArg)
        self.registerPythonClass("Gelanderholm", "OpenLxApp.Wall")

        self._insidePropUpdate = False

        self.setPropertyHeader(lxstr(Gelanderholm._propHeader), 548)
        self.setPropertyGroupName(lxstr(Gelanderholm._propGroupName), 549)

        self._length = self.registerPropertyDouble(Gelanderholm._lengthPropName,
                                                   3.0,
                                                   lx.Property.VISIBLE,
                                                   lx.Property.EDITABLE,
                                                   505)

        self._width = self.registerPropertyDouble(Gelanderholm._widthPropName,
                                                  0.1,
                                                  lx.Property.VISIBLE,
                                                  lx.Property.EDITABLE,
                                                  502)

        self._height = self.registerPropertyDouble(Gelanderholm._heightPropName,
                                                   2.0,
                                                   lx.Property.VISIBLE,
                                                   lx.Property.EDITABLE,
                                                   503)
    ...
    def onPropertyChanged(self, aPropertyName):
        if not self._insidePropUpdate:
            ...
            if aPropertyName == Gelanderholm._lengthPropName:
                self.setLength(self._length.getValue())
            elif aPropertyName == Gelanderholm._widthPropName:
                self.setWidth(self._width.getValue())
            elif aPropertyName == Gelanderholm._heightPropName:
...
if __name__ == "__main__":
    ...
    doc.registerPythonScript(Base.GlobalId("{426763B5-E2F7-4109-A190-0FAC1A4F4F55}"))

    try:
        cwColor = 3118
        element = Gelanderholm(doc)
        lx.setNewComponentByColorAndName(cwColor, Base.StringTool.toString("Dachgerüst Dachdeckerschutzwand"), element)
        ...
        prop1_value = Base.StringTool.toStlString(Base.PTranslator.get(548))  # "13_Dachgerüst_Dachdeckerschutzwand"
        ...
        cmd.CmdSetPropertySetDefinition("Pset_WallCommon", "reference", "13_Dachgerüst_Dachdeckerschutzwand", element).redo()
