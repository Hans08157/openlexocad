...
GUID_CLASS = Base.GlobalId("{DF2E4251-6CC7-4B64-B4EF-B3F1D2D6A36F}")
GUID_SCRPT = Base.GlobalId("{03202172-C439-46C7-9249-88BEECE59DC0}")
CW_COLOR = 3120
...
class DataStruct:
    def __init__(self, start_pnt, end_pnt, width, height, str_norm, end_norm, transform):
...
class SegmentBuilder:
    ...
    def getCutBeam(self, data):
        ...
        position = Geom.Ax2(Geom.Pnt(-beamLen[0], 0.0, 0.0), Geom.Dir(0, 0, 1))
...
class LSeitenschutz(lx.Railing):
    _propHeader = "_14LGiebelgerüstOhneArbeitsraum"
    _propGroupName = "_14LGiebelgerüstOhneArbeitsraumProperties"
    ...
    def __init__(self, aArg):
        ...
        self.registerPythonClass("LSeitenschutz", "OpenLxApp.Railing")

        self._insidePropUpdate = False

        self.setPropertyHeader(lxStr(LSeitenschutz._propHeader), 552)
        self.setPropertyGroupName(lxStr(LSeitenschutz._propGroupName), 553)

        self._width = self.registerPropertyDouble(LSeitenschutz._widthPropName,
                                                  0.2,
                                                  lx.Property.VISIBLE,
                                                  lx.Property.EDITABLE,
                                                  502)

        self._height = self.registerPropertyDouble(LSeitenschutz._heightPropName,
                                                   2.0,
                                                   lx.Property.VISIBLE,
                                                   lx.Property.EDITABLE,
                                                   503)
    ...
    def getPolylineData(self, lineSet):
        ...
        for edgeIndex in range(len(edges)):
            ...
            if edgeTypeRes.type == Geom.CurveType_LINE:
                ...
                lineData.append(
                    [PolylineData.SegmType_Arc, Geom.Pnt(p1Res.p), Geom.Pnt(p2Res.p), Geom.Pnt(p3Res.p)])
...
def main():
    ...
    if doc:
        ...
        cwColor = CW_COLOR
        element = LSeitenschutz(doc)
        lx.setNewComponentByColorAndName(cwColor, Base.StringTool.toString("Giebelgerüst ohne Arbeitsraum"), element)
        ...
        if thisScript.isDragAndDropped():
            ...
            if droppedOnElement:
                element.setLocalPlacement(droppedOnElement.getLocalPlacement())
        ...
        if geometry is None:
            ...
            if ok:
                ...
                if pickedElement:
                    element.setLocalPlacement(pickedElement.getLocalPlacement())
        ...
        prop1_value = Base.StringTool.toStlString(Base.PTranslator.get(550))  # "14_Giebelgerüst_ohne_Arbeitsraum"
        ...
        cmd.CmdSetPropertySetDefinition("Pset_RailingCommon", "reference", "14_Giebelgerüst_ohne_Arbeitsraum", element).redo()
