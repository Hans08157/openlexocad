...
class Abdeckung(lx.Slab):
    _classID = Base.GlobalId("{4E444741-892C-4E3C-99DD-8D8D06E5369F}")

    _propHeader = "_17Abdeckung"
    _propGroupName = "_17AbdeckungParameter"
    ...
    def getGlobalClassId(self):
        return Abdeckung._classID

    def __init__(self, aArg):
        lx.Slab.__init__(self, aArg)
        self.registerPythonClass("Abdeckung", "OpenLxApp.Slab")

        self._insidePropUpdate = False

        self.setPropertyHeader(lxstr(Abdeckung._propHeader), 558)
        self.setPropertyGroupName(lxstr(Abdeckung._propGroupName), 559)

        self._length = self.registerPropertyDouble(Abdeckung._lengthPropName,
                                                   2.0,
                                                   lx.Property.VISIBLE,
                                                   lx.Property.EDITABLE,
                                                   505)

        self._width = self.registerPropertyDouble(Abdeckung._widthPropName,
                                                  2.0,
                                                  lx.Property.VISIBLE,
                                                  lx.Property.EDITABLE,
                                                  502)

        self._height = self.registerPropertyDouble(Abdeckung._heightPropName,
                                                   0.2,
                                                   lx.Property.VISIBLE,
                                                   lx.Property.EDITABLE,
                                                   503)
    ...
    def onPropertyChanged(self, aPropertyName):
        if not self._insidePropUpdate:
            ...
            if aPropertyName == Abdeckung._lengthPropName:
                self.setLength(self._length.getValue())
            elif aPropertyName == Abdeckung._widthPropName:
                self.setWidth(self._width.getValue())
            elif aPropertyName == Abdeckung._heightPropName:
...
if __name__ == "__main__":
    ...
    doc.registerPythonScript(Base.GlobalId("{632C4200-159B-4F53-A904-9C3775436A8E}"))

    try:
        cwColor = 3123
        element = Abdeckung(doc)
        lx.setNewComponentByColorAndName(cwColor, Base.StringTool.toString("Abdeckung"), element)
        ...
        prop1_value = Base.StringTool.toStlString(Base.PTranslator.get(558))  # "17_Abdeckung"
        ...
        cmd.CmdSetPropertySetDefinition("Pset_SlabCommon", "reference", "17_Abdeckung", element).redo()
