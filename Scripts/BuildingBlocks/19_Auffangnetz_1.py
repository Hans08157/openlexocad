...
class Auffangnetz(lx.Slab):
    _classID = Base.GlobalId("{E1BE0CED-1CD3-458E-B020-2CDD8A9CC3B8}")

    _propHeader = "_19Auﬀangnetz"
    _propGroupName = "_19AuﬀangnetzParameter"
    ...
    def getGlobalClassId(self):
        return Auffangnetz._classID

    def __init__(self, aArg):
        lx.Slab.__init__(self, aArg)
        self.registerPythonClass("Auffangnetz", "OpenLxApp.Slab")

        self._insidePropUpdate = False

        self.setPropertyHeader(lxstr(Auffangnetz._propHeader), 562)
        self.setPropertyGroupName(lxstr(Auffangnetz._propGroupName), 563)

        self._length = self.registerPropertyDouble(Auffangnetz._lengthPropName,
                                                   3.0,
                                                   lx.Property.VISIBLE,
                                                   lx.Property.EDITABLE,
                                                   505)

        self._width = self.registerPropertyDouble(Auffangnetz._widthPropName,
                                                  2.5,
                                                  lx.Property.VISIBLE,
                                                  lx.Property.EDITABLE,
                                                  502)

        self._height = self.registerPropertyDouble(Auffangnetz._heightPropName,
                                                   0.1,
                                                   lx.Property.VISIBLE,
                                                   lx.Property.EDITABLE,
                                                   503)
    ...
    def onPropertyChanged(self, aPropertyName):
        if not self._insidePropUpdate:
            ...
            if aPropertyName == Auffangnetz._lengthPropName:
                self.setLength(self._length.getValue())
            elif aPropertyName == Auffangnetz._widthPropName:
                self.setWidth(self._width.getValue())
            elif aPropertyName == Auffangnetz._heightPropName:
...
if __name__ == "__main__":
    ...
    doc.registerPythonScript(Base.GlobalId("{28AFFDCA-3A92-43AF-A448-472BBC0DDA62}"))

    try:
        cwColor = 3125
        element = Auffangnetz(doc)
        lx.setNewComponentByColorAndName(cwColor, Base.StringTool.toString("Auffangnetz"), element)
        ...
        prop1_value = Base.StringTool.toStlString(Base.PTranslator.get(562))  # "19_Auffangnetz"
        ...
        cmd.CmdSetPropertySetDefinition("Pset_SlabCommon", "reference", "19_Auffangnetz", element).redo()
