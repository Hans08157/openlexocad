...
GUID_CLASS = Base.GlobalId("{ddf83ba6-ef11-41fe-8f19-15c9d3972102}")
GUID_SCRPT = Base.GlobalId("{d9639c40-4cf7-42cd-8bd0-fe2beb679432}")
CW_COLOR = 262
...
class RailingSegments(lx.Railing):
    ...
    def __init__(self, aArg):
        ...
        self._height = self.registerPropertyDouble(self._heightParamName, 1.03, lx.Property.NOT_VISIBLE,
        ...
        self._handrailType.addEntry(lxstr("Square"), -1)  # 489
        self._handrailType.addEntry(lxstr("Circle"), -1)  # 490
    ...
    def _fillAperture_Vertical(self, model, apertureStartPt, apertureVec):
        ...
        intEndPt = Geom.Pnt(intStartPt.x(), intStartPt.y(), intStartPt.z()+apertureHeight-0.5*botBeamV)
    ...
    def _placeLineStep(self, model, startPt, stepHLen, segmBase, segmHBase, segmRecCos, addSupport):
        ...
        # supportRadius = self._intermediateThickness.getValue()
        # hrRadiusV = 0.5*self._handrailHeight.getValue()
        topBeamH, topBeamV = self._sparWidth.getValue(), self._sparHeight.getValue()
        botBeamH, botBeamV = self._chordWidth.getValue(), self._sparHeight.getValue()
        # footPlateV = self._footplateSize[1]
        # apertureHeight = self._apertureSize[2]
        foundationHeight = self._foundationHeight.getValue() if self._isFoundation.getValue() else 0.0
        ...
        # Place top beam
        topEndHOffs = stepHLen - interThick * 2.0 * segmRecCos # !!!
        topEndOffs = topEndHOffs #* segmRecCos

        # zShift = bottomDist + foundationHeight + botBeamV + apertureHeight
        e_x = Geom.Vec(segmBase.x(), segmBase.y(), segmBase.z())
        e_y = Geom.Vec(-segmBase.y(), segmBase.x(), 0.0).normalized()
        e_z = e_x.crossed(e_y).normalized()
        if self._isSpar.getValue():
            zShift = self._height.getValue() - self._handrailHeight.getValue() - self._handrailSparGap.getValue() - topBeamV + foundationHeight
            topStartPt = Geom.Pnt(startPt.x(), startPt.y(), startPt.z() + zShift)
            topEndPt = baseVecTranslate(topStartPt, segmBase, topEndOffs)
            # start making transform: Matrix and translationPart
            
            Tr = self._getTransform(e_x, e_y, e_z, topStartPt)
            # finish making transform
            dtStr = DataStruct(sqType, topStartPt, topEndPt, 0.5*topBeamH, 0.5*topBeamV, -segmHBase, segmHBase, Tr)
        ...
        bottomStartPt = Geom.Pnt(startPt.x(), startPt.y(), startPt.z() + 0.5*botBeamV + bottomDist + foundationHeight)
        bottomEndPt = baseVecTranslate(bottomStartPt, segmBase, bottomEndOffs)
        Tr = self._getTransform(e_x, e_y, e_z, bottomStartPt)
        # finish making transform
        dtStr = DataStruct(sqType, bottomStartPt, bottomEndPt, 0.5*botBeamH, 0.5*botBeamV, -segmHBase, segmHBase, Tr)
    ...
    def _drawSupport(self, startSuppPt, segmBase, lenSupp, e_basis=None):
        ...
        dtStr = DataStruct(sqType, startSuppPt, endSuppPt, 0.5*supportWidth, 0.5*supportLength, strVec, endVec, Tr)
        beam_sup = self._poolContainer['supports'].createBeam(dtStr)
        self.addSubElement(beam_sup)
        # Footplate is added
    ...
    def _placeLineSegment(self, model, startPt, endPt, startVecs, endVecs, isFirst, isLast, isClosed):
        ...
        # finish making transform
        if isFirst:
            hrStartPt = baseVecTranslate(Geom.Pnt(startPt.x(), startPt.y(), startPt.z() + hrStartZDelta), startVec1, handrailLedge)
        else:    
            hrStartPt = Geom.Pnt(startPt.x(), startPt.y(), startPt.z() + hrStartZDelta)
        if isLast:
            hrEndPt = baseVecTranslate(Geom.Pnt(endPt.x(), endPt.y(), endPt.z() + hrStartZDelta), endVec1, handrailLedge)
        else:
            hrEndPt = Geom.Pnt(endPt.x(), endPt.y(), endPt.z() + hrStartZDelta)
        Tr = self._getTransform(e_x, e_y, e_z, hrStartPt)
        if hType == 0:
        ...
        beam = self._poolContainer['handrailbeams'].createBeam(dtStr)
        ...
        if isFirst and (not isClosed):
            ...
            self._drawSupport(startSuppPt, segmBase, lenSupp)

            buildStartOffs = 0.5*supportThickness * segmRecCos
    ...
    def _placeArcStep(self, model, startPt, endPt, addSupport):
        ...
        apertureStartPt = Geom.Pnt(startPt.x(), startPt.y(), startPt.z() + botBeamV + bottomDist + foundationHeight)
    ...
    def _placeArcSegment(self, model, arc, startVecs, endVecs, isFirst, isLast, isClosed):
        ...
        if isFirst and (not isClosed):
            ...
            self._drawSupport(startSuppPt, startSuppTang, lenSupp)
        ...
        if isLast and (not isClosed):
            ...
            self._drawSupport(endSuppStPt, endSuppTang, lenSupp)
        ...
        self._setHorCutHandrailBeams(model, listPnt, listVec[1], hrRadiusV, hrRadiusH, isFirst, isLast)

        self._setArrSupports(model, arc, listParam, listVec[0])

        if self._isSpar.getValue():
            self._setHorCutTopBeams(arc, listParam, listVec[1])

        self._setHorCutBottomBeams(model, arc, listParam, listVec[1])

        self._setHorCutFoundation(arc, listParam, listVec[1], isFirst, isLast)
