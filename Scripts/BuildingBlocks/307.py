...
class RailingSegments(lx.Railing):
    ...
    def __init__(self, aArg):
        ...
        self._supportThickness = self.registerPropertyDouble(self._supportThicknessParamName, 0.05, lx.Property.VISIBLE,
                                                           lx.Property.EDITABLE, -1)  # 487
        self._sparType = self.registerPropertyEnum(self._sparTypeParamName, 1, lx.Property.VISIBLE,
                                                       lx.Property.EDITABLE, -1)  # 488
        ...
        self._foundationHeight = self.registerPropertyDouble(self._foundationHeightParamName, 0.07, lx.Property.VISIBLE,
                                                           lx.Property.EDITABLE, -1)  # 491
        self._foundationLedge = self.registerPropertyDouble(self._foundationLedgeParamName, 0.0, lx.Property.VISIBLE,
                                                           lx.Property.EDITABLE, -1)
    ...
    def _fillAperture_Vertical(self, model, apertureStartPt, apertureVec):
        ...
        # print(0.0, 0.0, 1.0)
        # print(-normApertVec.x(), -normApertVec.y(), 0.0)
        # print(normApertVec.y(), -normApertVec.x(), 0.0)
        ...
        dtStr = DataStruct(1, intStartPt, intEndPt, 0.5*interWidth, intermRadius, strVec, endVec, Tr)
    ...
    def _placeLineSpars(self, startPt, stepHLen, segmBase, segmHBase, segmRecCos, addSupport, parts=3):
        ...
        sqType = self._sparType.getValue()
        if sqType == BeamFactory.BT_Circle:
            rR = 0.02415
        else:
            rR = 0.02
        e_x = Geom.Vec(segmBase.x(), segmBase.y(), segmBase.z())
        ...
        zShift = zShift - 0.32  # self._height.getValue() - self._handrailHeight.getValue() - self._handrailSparGap.getValue() - topBeamV + foundationHeight
    ...
    def _placeLineStep(self, model, startPt, stepHLen, segmBase, segmHBase, segmRecCos, addSupport, parts=3):  # 
        ...
        if parts:
            ...
            suppStartPt = baseVecTranslate(startPt, segmBase, stepHLen*3*0.05)
            self._drawSupport(suppStartPt, segmBase, lenSupp)
            for i in range(parts-1):
                suppStartPt = baseVecTranslate(suppStartPt, segmBase, stepHLen*3*0.3)
                self._drawSupport(suppStartPt, segmBase, lenSupp)
    ...
    def _placeArcSpars(self, arc, startParam, endParam, stepHLen, parts=3, 
        ...
        if start:
            ...
            self._drawSpar(sqType, startTopEdgePt, topStartPt, rR, rR, 
                        ...
                        startBase, Tr)
            # Tr = self._getTransform(e_x, e_y, e_z, topStartPt)
            # self._drawSpar(sqType, topStartPt, startTopEdgePt, rR, rR, 
            #             (-startBase).rotated(Geom.Ax1(startTopEdgePt, rot_dir), math.pi/4.), 
            #             startBase, Tr)
