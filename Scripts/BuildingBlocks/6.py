...
class Dormer4Elem(lx.Element):
    def getGlobalClassId(self):
        return Base.GlobalId("{1A0797BB-0BB1-4987-84FB-3CF79EAA39A6}")

    def __init__(self, aArg): 
        lx.Element.__init__(self, aArg)
        self.registerPythonClass("Dormer4Elem", "OpenLxApp.Element")
        # Register properties 
        self.setPropertyHeader(lxstr("Dormer4"), -1)
        self.setPropertyGroupName(lxstr("Dormer4 parameter"), -1)
        ...
        self._offset1 = self.registerPropertyDouble("Offset1", \
                                                    2.0, \
                                                    lx.Property.VISIBLE, \
                                                    lx.Property.EDITABLE, \
                                                    -1)

        self._offset2 = self.registerPropertyDouble("Offset2", \
                                                    2.0, \
                                                    lx.Property.VISIBLE, \
                                                    lx.Property.EDITABLE, \
                                                    -1)
    ...
    def modifyElem(self):
        block = self.createDormer4(doc)
        if block:
            self.setGeometry(block)

    def _updateGeometry(self):

        geom = self.createDormer4(doc)
    ...
    def onPropertyChanged(self, aPropertyName):
        ...
        elif aPropertyName == "Offset1":
            self.setOffset1(self._offset1.getValue())
        elif aPropertyName == "Offset2":
...
if __name__ == "__main__":   
    ...
    doc.registerPythonScript(Base.GlobalId("{9BBA4FD3-5663-4C9F-B19A-D1AEF25367CE}"))
    
    dormer4Elem = Dormer4Elem(doc)
    
    # Begin editing of the Element
    doc.beginEditing()

    b = dormer4Elem.createDormer4(doc)
    dormer4Elem.setGeometry(b)
    ...
    dormer4Elem.setLocalPlacement(pos)
