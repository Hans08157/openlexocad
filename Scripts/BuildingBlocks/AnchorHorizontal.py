...
class RuehlWandHorizontal(lx.PythonElement):
    _classID = "{B1BDF9A6-993F-4977-9AF4-FB55785B24FE}"
    _headerPropName = "Anker (Horizontal)"
    ...
    def getGlobalClassId(self):
        return Base.GlobalId(RuehlWandHorizontal._classID)

    def getScriptVersion(self):
        return 125  # Version 1.2.5

    def __init__(self, aArg):
        lx.PythonElement.__init__(self, aArg)
        print("Initializing Ruehlwand script...")
        self.registerPythonClass("RuehlWandHorizontal", "OpenLxApp.PythonElement")

        # Register properties
        self.setPropertyHeader(lxstr(RuehlWandHorizontal._headerPropName), -1)
        self.setPropertyGroupName(lxstr(RuehlWandHorizontal._groupPropName), -1)

        self._nailLength = self.registerPropertyDouble(RuehlWandHorizontal._nailLengthPropName, 5.0,
                                                       lx.Property.VISIBLE,
                                                       lx.Property.EDITABLE, 1)
        self._nailRadius = self.registerPropertyDouble(RuehlWandHorizontal._nailRadiusPropName, 0.075,
                                                       lx.Property.VISIBLE,
        ...
        self._nailHeadRadius = self.registerPropertyDouble(RuehlWandHorizontal._nailHeadRadiusName, 0.10,
                                                           lx.Property.VISIBLE,
                                                           lx.Property.EDITABLE, 20)
        self._nailHeadLength = self.registerPropertyDouble(RuehlWandHorizontal._nailHeadLengthName, 0.25,
                                                           lx.Property.VISIBLE,
        ...
        self._nailPlateWidth = self.registerPropertyDouble(RuehlWandHorizontal._nailPlateWidthName, 0.40,
                                                           lx.Property.VISIBLE,
                                                           lx.Property.EDITABLE, 23)
        self._nailPlateHeight = self.registerPropertyDouble(RuehlWandHorizontal._nailPlateHeightName, 0.40,
                                                            lx.Property.VISIBLE,
                                                            lx.Property.EDITABLE, 24)
        self._nailPlateThickness = self.registerPropertyDouble(RuehlWandHorizontal._nailPlateThicknessName, 0.05,
                                                               lx.Property.VISIBLE,
                                                               lx.Property.EDITABLE, 25)

        self._bottomLength = self.registerPropertyDouble(RuehlWandHorizontal._bottomLengthPropName, 1.0,
                                                         lx.Property.VISIBLE,
                                                         lx.Property.EDITABLE, 3)
        self._bottomRadius = self.registerPropertyDouble(RuehlWandHorizontal._bottomRadiusPropName, 0.12,
                                                         lx.Property.VISIBLE,
                                                         lx.Property.EDITABLE, 4)
        self._angle = self.registerPropertyDouble(RuehlWandHorizontal._anglePropName, 0.0, lx.Property.VISIBLE,
                                                  lx.Property.EDITABLE, 5)
        self._nailColor = self.registerPropertyColor(RuehlWandHorizontal._nailColorName, Base.Color_fromCdwkColor(34),
                                                     lx.Property.VISIBLE,
                                                     lx.Property.EDITABLE, 6)
        self._injectionColor = self.registerPropertyColor(RuehlWandHorizontal._injectionColorName,
    ...
    def _createNail(self):
        ...
        # eas.setPosition(Geom.Ax2(Geom.Pnt(0.0, nail_len, 0.0), Geom.Dir(0, 1, 0)))
    ...
    def _createBottom(self):
        ...
        positionAngle = math.radians(90.0)
        rotateAxis = Geom.Ax1(Geom.Pnt(0, 0, 0), Geom.Dir(1, 0, 0))
        botElem.rotate(rotateAxis, -(angle + positionAngle), Geom.CoordSpace_WCS)
    ...
    def onPropertyChanged(self, aPropertyName):
        if aPropertyName == RuehlWandHorizontal._nailRadiusPropName:
            self.setNailRadius(self._nailRadius.getValue())
        elif aPropertyName == RuehlWandHorizontal._nailLengthPropName:
            self.setNailLength(self._nailLength.getValue())
        elif aPropertyName == RuehlWandHorizontal._bottomRadiusPropName:
            self.setBottomRadius(self._bottomRadius.getValue())
        elif aPropertyName == RuehlWandHorizontal._bottomLengthPropName:
            self.setBottomLength(self._bottomLength.getValue())
        elif aPropertyName == RuehlWandHorizontal._anglePropName:
            self.setAngle(self._angle.getValue())
        elif aPropertyName == RuehlWandHorizontal._nailColorName:
            self.setNailColor(self._nailColor.getValue())
        elif aPropertyName == RuehlWandHorizontal._injectionColorName:
            self.setInjectionColor(self._injectionColor.getValue())
        elif aPropertyName == RuehlWandHorizontal._nailHeadRadiusName:
            self.setNailHeadRadius(self._nailHeadRadius.getValue())
        elif aPropertyName == RuehlWandHorizontal._nailHeadLengthName:
            self.setNailHeadLength(self._nailHeadLength.getValue())
        elif aPropertyName == RuehlWandHorizontal._nailPlateWidthName:
            self.setNailPlateWidth(self._nailPlateWidth.getValue())
        elif aPropertyName == RuehlWandHorizontal._nailPlateHeightName:
            self.setNailPlateHeight(self._nailPlateHeight.getValue())
        elif aPropertyName == RuehlWandHorizontal._nailPlateThicknessName:
            self.setNailPlateThickness(self._nailPlateThickness.getValue())
        elif aPropertyName == RuehlWandHorizontal._showNailHeadName:
            self.setShowHead(self._showNailHead.getValue())
        elif aPropertyName == RuehlWandHorizontal._showHeadPlateName:
...
if __name__ == "__main__":
    ...
    doc.registerPythonScript(Base.GlobalId("{4AD77A76-1EFA-4303-9B9F-8AC27ABA62B6}"))

    try:
        compound = RuehlWandHorizontal(doc)
        with EditMode(doc):
            compound.setDiffuseColor(Base.Color_fromCdwkColor(39))

        # Creating PropertySets
        cmd.CmdSetPropertySetDefinition(property_set_name, "Angle", compound._angle.getValue(), compound).redo()
        cmd.CmdSetPropertySetDefinition(property_set_name, "Nail Length", compound._nailLength.getValue(), compound).redo()
        cmd.CmdSetPropertySetDefinition(property_set_name, "Nail Radius", compound._nailRadius.getValue(), compound).redo()
        cmd.CmdSetPropertySetDefinition(property_set_name, "Injection Length", compound._bottomLength.getValue(), compound).redo()
        cmd.CmdSetPropertySetDefinition(property_set_name, "Injection Radius", compound._bottomRadius.getValue(), compound).redo()
