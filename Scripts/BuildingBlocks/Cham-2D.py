...
CHAMBER_TYPES = {
    "2D": {
        "bottom_height": 0.10,
        "external_length": 2.29,
        "external_width": 1.24,
        "internal_length": 2.05,
        "internal_width": 1.00,
        "heights": {
            "2D SF06": 0.60,
            "2D SF09": 0.90,
        ...
        "tapes": {
            ...
            "CHB CON 124 120": {
                ...
                "length": 1.22,
                "height": 0.40,
                "inner_diameter": 0.60,  # ! plus 4 CV T60 tapes
            },
            "CHB CV T10 120 030": {  # ! 8x
                "width": 1.20,
                "length": 0.30,
                "height": 0.065,
            },
            "CHB CV T25 120 030": {  # ! 8x
                "width": 1.20,
                "length": 0.30,
                "height": 0.10,
            },
            "CHB CV T60 120 030": {  # ! 8x
                "width": 1.20,
                "length": 0.30,
                "height": 0.13,
            },
...
class DistributionChamber2D(lx.DistributionChamberElement):
    _classID = "{329d86d4-6578-4748-a420-2419c702cd99}"
    _headerPropName = "Distribution Chamber 2D"
    ...
    def getGlobalClassId(self) -> Base.GlobalId:
        return Base.GlobalId(DistributionChamber2D._classID)

    def __init__(self, aArg) -> None:
        lx.DistributionChamberElement.__init__(self, aArg)
        self.registerPythonClass("DistributionChamber2D", "OpenLxApp.DistributionChamberElement")
        # Register properties
        self.setPropertyHeader(lxstr(DistributionChamber2D._headerPropName), -1)
        self.setPropertyGroupName(lxstr(DistributionChamber2D._groupPropName), -1)

        self._tapeType = self.registerPropertyEnum(DistributionChamber2D._tapeTypePropName, 0,
                                                        lx.Property.VISIBLE,
                                                        lx.Property.EDITABLE, -1)
        self._tapeType.setEmpty()
        for key in list(CHAMBER_TYPES["2D"]["tapes"].keys()):
            self._tapeType.addEntry(lxstr(key), -1)

        self._heights = self.registerPropertyEnum(DistributionChamber2D._heightsPropName, 0,
                                                       lx.Property.VISIBLE,
                                                       lx.Property.EDITABLE, -1)
        self._heights.setEmpty()
        for key in list(CHAMBER_TYPES["2D"]["heights"].keys()):
            self._heights.addEntry(lxstr(key), -1)

        self._showBottom = self.registerPropertyBool(
            DistributionChamber2D._showBottomPropName,
            False,
            lx.Property.VISIBLE,
    ...
    def carveTheNichesInChamber(self, chamber_element: lx.Element) -> lx.SubElement:
        ...
        doc = self.getDocument()
        niche_height = 0.40
        external_length: float = CHAMBER_TYPES["2D"]["external_length"]
        internal_length: float = CHAMBER_TYPES["2D"]["internal_length"]
        external_width: float = CHAMBER_TYPES["2D"]["external_width"]
        internal_width: float = CHAMBER_TYPES["2D"]["internal_width"]
        niche_width: float = external_width - internal_width
        ...
        firstBlock.setXLength(niche_height)
        firstBlock.setYLength(niche_height)
        firstBlock.setZLength(niche_height)
        x_cut_offset = external_length / 3.0
        x_additional_offset = niche_height * 0.5
        firstBlockElement = lx.Element.createIn(doc)
        firstBlockElement.setGeometry(firstBlock)
        firstBlockElement.translate(
            Geom.Vec(
                x_cut_offset - niche_height - x_additional_offset,
                -niche_height + cutting_depth,
                0.0
            ),
            Geom.CoordSpace_WCS
        )
        ...
        secondBlock.setXLength(niche_height)
        secondBlock.setYLength(niche_height)
        secondBlock.setZLength(niche_height)
        secondBlockElement = lx.Element.createIn(doc)
        secondBlockElement.setGeometry(secondBlock)
        secondBlockElement.translate(
            Geom.Vec(
                x_cut_offset * 2 - niche_height - x_additional_offset,
                -niche_height + cutting_depth,
                0.0
            ),
            Geom.CoordSpace_WCS
        ...
        thirdBlock.setXLength(niche_height)
        thirdBlock.setYLength(niche_height)
        thirdBlock.setZLength(niche_height)
        thirdBlockElement = lx.Element.createIn(doc)
        thirdBlockElement.setGeometry(thirdBlock)
        thirdBlockElement.translate(
            Geom.Vec(
                x_cut_offset * 3 - niche_height - x_additional_offset,
                -niche_height + cutting_depth,
                0.0
            ),
            Geom.CoordSpace_WCS
        ...
        fourthBlock.setXLength(niche_height)
        fourthBlock.setYLength(niche_height)
        fourthBlock.setZLength(niche_height)
        fourthBlockElement = lx.Element.createIn(doc)
        fourthBlockElement.setGeometry(fourthBlock)
        fourthBlockElement.translate(
            Geom.Vec(
                x_cut_offset - niche_height - x_additional_offset,
                external_width - cutting_depth,
                0.0
            ),
            Geom.CoordSpace_WCS
        )

        fifthBlock = lx.Block.createIn(doc)
        fifthBlock.setXLength(niche_height)
        fifthBlock.setYLength(niche_height)
        fifthBlock.setZLength(niche_height)
        fifthBlockElement = lx.Element.createIn(doc)
        fifthBlockElement.setGeometry(fifthBlock)
        fifthBlockElement.translate(
            Geom.Vec(
                x_cut_offset * 2 - niche_height - x_additional_offset,
                external_width - cutting_depth,
                0.0
        ...
        sixthBlock = lx.Block.createIn(doc)
        sixthBlock.setXLength(niche_height)
        sixthBlock.setYLength(niche_height)
        sixthBlock.setZLength(niche_height)
        sixthBlockElement = lx.Element.createIn(doc)
        sixthBlockElement.setGeometry(sixthBlock)
        sixthBlockElement.translate(
            Geom.Vec(
                x_cut_offset * 3 - niche_height - x_additional_offset,
                external_width - cutting_depth,
                0.0
        ...
        seventhBlock = lx.Block.createIn(doc)
        seventhBlock.setXLength(niche_height)
        seventhBlock.setYLength(niche_height)
        seventhBlock.setZLength(niche_height)
        seventhBlockElement = lx.Element.createIn(doc)
        seventhBlockElement.setGeometry(seventhBlock)
        seventhBlockElement.translate(Geom.Vec(-niche_height + cutting_depth, niche_height * 0.5, 0.0),
                                    Geom.CoordSpace_WCS)

        eightBlock = lx.Block.createIn(doc)
        eightBlock.setXLength(niche_height)
        eightBlock.setYLength(niche_height)
        eightBlock.setZLength(niche_height)
        eightBlockElement = lx.Element.createIn(doc)
        eightBlockElement.setGeometry(eightBlock)
        eightBlockElement.translate(Geom.Vec(external_length - cutting_depth, niche_height * 0.5, 0.0),
                                    Geom.CoordSpace_WCS)
        ...
        resultingBlocks5 = lx.vector_Element()
        resultingBlocks6 = lx.vector_Element()
        resultingBlocks7 = lx.vector_Element()
        resultingBlocks8 = lx.vector_Element()
        ...
        if lx.bop_cut(resultingBlocks4[0], fifthBlockElement, resultingBlocks5) != 0:
            print("Error: bop_cut failed")

        if lx.bop_cut(resultingBlocks5[0], sixthBlockElement, resultingBlocks6) != 0:
            print("Error: bop_cut failed")

        if lx.bop_cut(resultingBlocks6[0], seventhBlockElement, resultingBlocks7) != 0:
            print("Error: bop_cut failed")

        if lx.bop_cut(resultingBlocks7[0], eightBlockElement, resultingBlocks8) != 0:
            print("Error: bop_cut failed")
        ...
        doc.removeObject(fifthBlockElement)
        doc.removeObject(sixthBlockElement)
        doc.removeObject(seventhBlockElement)
        doc.removeObject(eightBlockElement)
        doc.removeObject(chamber_element)

        transform = resultingBlocks8[0].getTransform()
        geometry = resultingBlocks8[0].getGeometry()
    ...
    def _get_tape_type_string(self) -> str:
        ...
        tape_type = list(CHAMBER_TYPES["2D"]["tapes"].keys())
    ...
    def _calculate_height_offset_for_tape(self, tape_type: str) -> float:
        def get_height_type_string() -> str:
            ...
            height_type = list(CHAMBER_TYPES["2D"]["heights"].keys())
            height_type: str = height_type[height_type_index]
            return height_type
    
        base_height: float = CHAMBER_TYPES["2D"]["bottom_height"]
        current_chamber_height: float = CHAMBER_TYPES["2D"]["heights"][get_height_type_string()]
    ...
    def create_tape_2D_B125(self) -> lx.SubElement:  # "CHB E CDB 229 124N"
        '''Build CHB E CDB 229 124N tape SubElement'''
        chamber_tapes_dict = CHAMBER_TYPES["2D"]["tapes"]["CHB E CDB 229 124N"]
        ...
        external_length = CHAMBER_TYPES["2D"]["external_length"]
        external_width = CHAMBER_TYPES["2D"]["external_width"]
        doc = self.getDocument()
        
        starting_height: float = self._calculate_height_offset_for_tape("CHB E CDB 229 124N")
        ...
        tape_base_with_hole.translate(
            Geom.Vec(
                # -(length - external_length) * 0.5,
                0.0,
                # -(width - external_width) * 0.5,
                0.0,
                starting_height
            ),
            Geom.CoordSpace_WCS
        ...
        tape_3: lx.SubElement = self.create_simplified_block(
            width=inner_width + 0.08,
            length=inner_length * 0.25 + 0.04,
            height=0.02,
            return_elem_type=ElemTypeEnum.LX_SUB_ELEMENT
        )
        tape_4: lx.SubElement = self.create_simplified_block(
            width=inner_width + 0.08,
            length=inner_length * 0.25 + 0.04,
            height=0.02,
            return_elem_type=ElemTypeEnum.LX_SUB_ELEMENT
        )
        tapes_x_offset = inner_length * 0.25
        tape_1.translate(Geom.Vec(0.04, 0.08, starting_height + height), Geom.CoordSpace_WCS)
        tape_1.setDiffuseColor(Base.Color_fromCdwkColor(15))
        tape_2.translate(Geom.Vec(tapes_x_offset + 0.08, 0.08, starting_height + height), Geom.CoordSpace_WCS)
        tape_2.setDiffuseColor(Base.Color_fromCdwkColor(15))
        tape_3.translate(Geom.Vec(tapes_x_offset * 2 + 0.12, 0.08, starting_height + height), Geom.CoordSpace_WCS)
        tape_3.setDiffuseColor(Base.Color_fromCdwkColor(15))
        tape_4.translate(Geom.Vec(tapes_x_offset * 3 + 0.16, 0.08, starting_height + height), Geom.CoordSpace_WCS)
        tape_4.setDiffuseColor(Base.Color_fromCdwkColor(15))
        self.addSubElement(tape_1)
        self.addSubElement(tape_2)
        self.addSubElement(tape_3)
        self.addSubElement(tape_4)

        return tape_base_with_hole

    def create_tape_2D_B400(self) -> lx.SubElement:  # "CHB E CDD 240 127"
        '''Build CHB E CDD 240 127 tape SubElement'''
        chamber_tapes_dict = CHAMBER_TYPES["2D"]["tapes"]["CHB E CDD 240 127"]
        width: float = chamber_tapes_dict["width"]
        height: float = chamber_tapes_dict["height"]
        length: float = chamber_tapes_dict["length"]
        inner_width = chamber_tapes_dict["inner_width"]
        ...
        starting_height: float = self._calculate_height_offset_for_tape("CHB E CDD 240 127")
        ...
        tape_base_with_hole: lx.SubElement = self.prepare_chamber(hardElement=hard_block, softElement=soft_block,
                                                                  return_elem_type=ElemTypeEnum.LX_SUB_ELEMENT)
        tape_base_with_hole.translate(
            Geom.Vec(-(length - external_length) * 0.5, -(width - external_width) * 0.5, starting_height),
            Geom.CoordSpace_WCS
        )
        tape_base_with_hole.setDiffuseColor(Base.Color_fromCdwkColor(18))
        ...
        tapes_x_offset = inner_length * 0.25
        tape_1.translate(Geom.Vec(0.04, 0.1, starting_height + height), Geom.CoordSpace_WCS)
        tape_1.setDiffuseColor(Base.Color_fromCdwkColor(15))
        tape_2.translate(Geom.Vec(tapes_x_offset + 0.08, 0.1, starting_height + height), Geom.CoordSpace_WCS)
        tape_2.setDiffuseColor(Base.Color_fromCdwkColor(15))
        tape_3.translate(Geom.Vec(tapes_x_offset * 2 + 0.12, 0.1, starting_height + height), Geom.CoordSpace_WCS)
        tape_3.setDiffuseColor(Base.Color_fromCdwkColor(15))
        tape_4.translate(Geom.Vec(tapes_x_offset * 3 + 0.16, 0.1, starting_height + height), Geom.CoordSpace_WCS)
        tape_4.setDiffuseColor(Base.Color_fromCdwkColor(15))
        self.addSubElement(tape_1)
        self.addSubElement(tape_2)
        self.addSubElement(tape_3)
    ...
    def create_tape_con_124_120(self) -> lx.SubElement:  # "CHB CON 124 120"
        ...
        chamber_types_dict = CHAMBER_TYPES["2D"]["tapes"]["CHB CON 124 120"]
        ...
        chamber_length: float = CHAMBER_TYPES["2D"]["external_length"]
        chamber_width: float = CHAMBER_TYPES["2D"]["external_width"]
        ...
        final_element.translate(Geom.Vec(chamber_length - length * 0.5, chamber_width * 0.5, anchor_height), Geom.CoordSpace_WCS)
        final_element.setDiffuseColor(Base.Color_fromCdwkColor(18))
        
        return final_element

    def create_tape_chb_cvt_10(self, tapes_quantity: int, starting_length: float) -> lx.SubElement: # CHB.CV.T10.120/030
        def prepare_single_cvt10_tape_sub_element() -> lx.SubElement:
            '''Build CHB.CV.T10.120/030 tape SubElement'''
            chamber_tapes_dict = CHAMBER_TYPES["2D"]["tapes"]["CHB CV T10 120 030"]
            width: float = chamber_tapes_dict["width"]
            height: float = chamber_tapes_dict["height"]
            length: float = chamber_tapes_dict["length"]
            doc = self.getDocument()
        ...
        width = CHAMBER_TYPES["2D"]["tapes"]["CHB CV T10 120 030"]["width"]
        external_width: float = CHAMBER_TYPES["2D"]["external_width"]
        width_offset: float = (external_width - width) * 0.5
        
        doc = self.getDocument()
        starting_height: float = self._calculate_height_offset_for_tape("CHB CV T10 120 030")
        length_offset: float = 0.0
        for _ in range(0, tapes_quantity):
            tape_element = prepare_single_cvt10_tape_sub_element()
            tape_element.translate(Geom.Vec(starting_length + length_offset, width_offset, starting_height), Geom.CoordSpace_WCS)
            length_offset += 0.30
            self.addSubElement(tape_element)
      
    def create_tape_chb_cvt_25(self, tapes_quantity: int, starting_length: float) -> lx.SubElement: # CHB.CV.T25.120/030
        def prepare_single_cvt25_tape_sub_element() -> lx.SubElement:
            '''Build CHB.CV.T25.120/030 tape SubElement'''
            chamber_tapes_dict = CHAMBER_TYPES["2D"]["tapes"]["CHB CV T25 120 030"]
            width: float = chamber_tapes_dict["width"]
            height: float = chamber_tapes_dict["height"]
            length: float = chamber_tapes_dict["length"]
            doc = self.getDocument()
        ...
        width = CHAMBER_TYPES["2D"]["tapes"]["CHB CV T25 120 030"]["width"]
        external_width: float = CHAMBER_TYPES["2D"]["external_width"]
        width_offset: float = (external_width - width) * 0.5
    ...
    def create_tape_chb_cvt_60(self, tapes_quantity: int, starting_length: float) -> lx.SubElement: # CHB.CV.T60.120/030
        def prepare_single_cvt60_tape_sub_element() -> lx.SubElement:
            '''Build CHB.CV.T60.120/030 tape SubElement'''
            chamber_tapes_dict = CHAMBER_TYPES["2D"]["tapes"]["CHB CV T60 120 030"]
            width: float = chamber_tapes_dict["width"]
            height: float = chamber_tapes_dict["height"]
            length: float = chamber_tapes_dict["length"]
            doc = self.getDocument()
        ...
        width = CHAMBER_TYPES["2D"]["tapes"]["CHB CV T60 120 030"]["width"]
        external_width: float = CHAMBER_TYPES["2D"]["external_width"]
        width_offset: float = (external_width - width) * 0.5
        doc = self.getDocument()
        ...
        for _ in range(0, tapes_quantity):
            ...
            translation_vec = Geom.Vec(starting_length + length_offset, width_offset, starting_height)
    ...
    def _createGeometry(self)  -> None:
        ...
        external_width: float = CHAMBER_TYPES["2D"]["external_width"]
        internal_width: float = CHAMBER_TYPES["2D"]["internal_width"]
        bottom_height: float = CHAMBER_TYPES["2D"]["bottom_height"]
        show_bottom: bool = self._showBottom.getValue()
        
        current_height_param: float = self._heights.getValue()
        # convert dict keys to list
        height_param_height_type = list(CHAMBER_TYPES["2D"]["heights"].keys())[current_height_param]
        height = CHAMBER_TYPES["2D"]["heights"][height_param_height_type]
        ...
        if all([external_length, internal_length, external_width, internal_width, height]):
            ...
            if tape_type == "CHB E CDB 229 124N":
                tape = self.create_tape_2D_B125()
                self.addSubElement(tape)
            elif tape_type == "CHB E CDD 240 127":
                tape = self.create_tape_2D_B400()
                self.addSubElement(tape)
            elif tape_type == "CHB CON 124 120":
                tape = self.create_tape_con_124_120()
                self.addSubElement(tape)
                self.create_tape_chb_cvt_60(tapes_quantity=4, starting_length=-0.13)
            elif tape_type == "CHB CV T10 120 030":
                self.create_tape_chb_cvt_10(tapes_quantity=8, starting_length=-0.055)
            elif tape_type == "CHB CV T25 120 030":
                self.create_tape_chb_cvt_25(tapes_quantity=8, starting_length=-0.055)
            elif tape_type == "CHB CV T60 120 030":
                self.create_tape_chb_cvt_60(tapes_quantity=8, starting_length=-0.055)
            else:
    ...
    def onPropertyChanged(self, aPropertyName):
        if aPropertyName == DistributionChamber2D._tapeTypePropName:
            self.setTapeType(self._tapeType.getValue())
        elif aPropertyName == DistributionChamber2D._heightsPropName:
            self.setHeights(self._heights.getValue())
        elif aPropertyName == DistributionChamber2D._showBottomPropName:
...
if __name__ == "__main__":
    ...
    doc.registerPythonScript(Base.GlobalId("{42d5df15-5ac9-476c-a582-d659b31de733}"))

    try:
        compound = DistributionChamber2D(doc)
